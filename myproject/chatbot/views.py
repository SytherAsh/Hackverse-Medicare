...
def get_qa_chain(pdf_name: str, groq_api: str):
...
def upload_pdf(request):
    if request.method == 'POST' and request.FILES.get('pdf_file'):
        try:
            ...
            # Redirect to summarization report
            return redirect('summarization_report', pdf_name=pdf_name)
...
def generate_summary(pdf_name: str):
    """Generate a summary for the processed PDF"""
    try:
        ...
        # Generate summary using a simple summarization model or logic
       
        # Initialize LLM
        llm = HuggingFaceHub(repo_id="facebook/bart-large-cnn", model_kwargs={"temperature": 0.3})
        summarize_chain = load_summarize_chain(llm, chain_type="map_reduce")
        
        summary = summarize_chain.run(full_text)
        ...
        raise
def summarization_report(request, pdf_name: str):
    """View for the summarization report"""
    try:
        # Generate the summary
        summary = generate_summary(pdf_name)
        
        # Pass the summary to the template
        return render(request, 'chatbot/summary.html', {'pdf_name': pdf_name, 'summary': summary})
    except Exception as e:
        print(f"Error in summarization_report: {str(e)}")
        return JsonResponse({'error': str(e)}, status=500)
